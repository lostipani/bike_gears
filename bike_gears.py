...
inpRear = dash.dcc.Input(
    value="11, 13, 15, 17, 19, 21, 23, 25, 28", type="text"
)
...
app.layout = dash.html.Div(
    ...
    children=[
        dash.html.H1(
            children="Bike: gear ratios",
            style={"font-family": "Arial, sans-serif"},
        ),  # title
        dash.html.Div(
            ["Front chainrings", inpFront, "Rear sprockets", inpRear],
            style={"font-family": "Arial, sans-serif"},
        ),  # input fields
...
def update_fig(inp_front, inp_rear):
    ...
    def get_gear_ratio(frontGears: list, rearGears: list) -> np.ndarray:
        ...
        for i in range(n_chainring):
            for j in range(n_sprocket):
                gear_ratio[i, j] = np.format_float_positional(
                    float(front_gears[i] / rear_gears[j]), precision=2
                )
        return gear_ratio
    ...
    fig.update_layout(
        xaxis=dict(
            tickmode="array", tickvals=xticks, ticktext=np.array(rear_gears)
        ),
        yaxis=dict(
            tickmode="array", tickvals=yticks, ticktext=np.array(front_gears)
        ),
    )
    fig.add_shape(
        type="rect",
